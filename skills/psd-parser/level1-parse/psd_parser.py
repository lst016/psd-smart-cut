...
class PSDDocument:
    ...
    def __post_init__(self):
        if not self.width and self.pages:
            self.width = max((page.width for page in self.pages), default=0)
        if not self.height and self.pages:
            self.height = max((page.height for page in self.pages), default=0)

        computed_layers = sum(page.layer_count or len(page.layers) for page in self.pages)
        if not self.total_layers:
            self.total_layers = self.layer_count or computed_layers
        if not self.layer_count:
            self.layer_count = self.total_layers or computed_layers
